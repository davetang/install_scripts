...
if [[ ${KERNEL} == Linux ]]; then
   if [[ ${ARCH} == x86_64 ]]; then
      ...
      >&2 echo ${KERNEL} ${ARCH} detected
      URL=https://github.com/jgm/pandoc/releases/download/${VER}/pandoc-${VER}-linux-arm64.tar.gz
   else
   ...
   if [[ ${ARCH} == x86_64 ]]; then
      >&2 echo ${KERNEL} ${ARCH} detected
      URL=https://github.com/jgm/pandoc/releases/download/${VER}/pandoc-${VER}-x86_64-macOS.zip
   elif [[ ${ARCH} == arm64 ]]; then
      >&2 echo ${KERNEL} ${ARCH} detected
      URL=https://github.com/jgm/pandoc/releases/download/${VER}/pandoc-${VER}-arm64-macOS.zip
   else
      >&2 echo Unsupported architecture: ${ARCH}
   fi
else
